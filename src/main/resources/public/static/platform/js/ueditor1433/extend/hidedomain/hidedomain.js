UE.registerUI('hidedomain', function(editor, uiName) {

	// 创建dialog
	var dialog = new UE.ui.Dialog({
		// 指定弹出层中页面的路径，这里只能支持页面,因为跟addCustomizeDialog.js相同目录，所以无需加路径
		iframeUrl : __ctx + '/static/platform/js/ueditor1433/extend/hidedomain/hidedomainDialog.jsp',
		// 需要指定当前的编辑器实例
		editor : editor,
		// 指定dialog的名字
		name : uiName,
	...
	var btn = new UE.ui.Button({
		...
		cssRules : 'background-position: -1020px -40px;',
	...
	//返回btn按钮是因为菜单上展示的是按钮，你也可以返回其他html(并没试过)或者直接不返回，那么在菜单上就不会出现按钮
	return btn;
}/*

...
<script type="text/javascript">
		...
		function getMenuItem(){
			...
			var orgauthPerms = $("#orgAuth").children('option:selected').attr("orgauthPerms");
			var posPerms = $("#orgAuth").children('option:selected').attr("posPerms");
			var items=[];
			...
			if(orgauthPerms !=""){
				items.push({text : '设置管理员',click: OrgTree.orgAuth,alias:''}); 
			}
...
<body>
	...
	<c:choose>
		...
		<div position="left" title="组织机构管理" id="rogTree" style="height: 100%; width: 100% !important;">
				<div style="width:100%;">
			        <select id="orgAuth" style="width:99.8% !important;" onchange="javascript:OrgTree.loadTree();">  
			              <c:forEach var="orgAuth" items="${orgAuthList}">  
			         			<option value="${orgAuth.orgId}" authId="${orgAuth.id}" dimId="${orgAuth.dimId}" orgPerms="${orgAuth.orgPerms}" orgauthPerms="${orgAuth.orgauthPerms}" posPerms="${orgAuth.posPerms}" >${orgAuth.orgName}—[${orgAuth.dimName}]</option>  
			        	  </c:forEach>  
			        </select>
				</div>
			...
			<ul id="orgTree" class="ztree" style="height: 100%;width: 100%"></ul>
		</div>

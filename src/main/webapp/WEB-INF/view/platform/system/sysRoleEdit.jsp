        ...
        <div class="panel-body">
                ...
                <form id="sysRoleForm" method="post" action="save.ht">
                    <table class="table-detail" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr>
                            <th><span>角色分类:</span></th>
                            <td>
                                <input id="category" name="category" value="${sysRole.category}" class="inputText"/> 
                                <select onchange="selectCategory(this)" class="inputText">
                                    <option value="">请选择</option>
                                    <c:forEach items="${categoryList}" var="catName">
                                        <option value="${catName}" <c:if test="${sysRole.category==catName}">selected</c:if>>${catName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>

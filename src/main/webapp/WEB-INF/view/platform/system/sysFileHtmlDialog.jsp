...
<script type="text/javascript">
	...
	app.controller('uploadCtrl',[ '$scope', 'FileUploader', function($scope, FileUploader) {
				...
				uploader.filters.push({
					...
					fn : function(item, options) {
						...
						if(result){
							...
							if($("div.l-dialog-title:contains('最多只能上传')").length<=0){
								$.ligerDialog.alertExt("提示信息","最多只能上传"+max+"个文件");
							}
						}
					}
				});	
			...
			if(true){
				//上传文件的文件名字符长度过滤器
				uploader.filters.push({
					name : 'nameCharSizeFilter',
					fn : function(item, options) {
						var result = item.name.length<=100;
						!result&&($.ligerDialog.alertExt("提示信息","文件名字符长度不能超过100"));
						return result;
					}
				});	
			}
			uploader.onSuccessItem = function(fileItem, response) {
				fileItem.json = {fileId:response.fileId,fileName:response.fileName,extName:response.extName};
	        };
		}]);
</script>
</head>

	...
	<script type="text/javascript">
		...
		function heightChanged(options){
			if(options){
			    ...
			    if(${isShowRole}) 		$("#SEARCH_BY_ORG").height(335 - 29);
		...
		function orgTreeOnAsyncSuccess(event, treeId, treeNode, msg) {
			if(treeNode){
			  		...
			  		org_Tree.updateNode(treeNode);
			  		 ...
			  		 for(var i=0;i<children.length;i++){
			  			 if(children[i].orgId==currentOrgId && initFlag==0){
			  				orgTree.setting.callback.onClick(null,"SEARCH_BY_ORG",children[i]);
				...
				//初始化时为undinfined
				 if(type!='script'){
				  setTimeout(function(){
					orgTree = $.fn.zTree.getZTreeObj("SEARCH_BY_ORG");
		...
		function load_Rol_Tree(){
			...
			function add(data) {
				...
				if(len>0) return;
				var _trs=$("#sysUserList").find("tr[id^='user_']");
				if(_trs.length>80){
					if($("div.l-dialog-title:contains('选择人数不能超过80')").length<=0){
						$.ligerDialog.alertExt("提示信息","选择人数不能超过80");
					}
					return;
				}

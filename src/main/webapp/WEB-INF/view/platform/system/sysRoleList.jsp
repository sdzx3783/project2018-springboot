			...
			<div class="hide-panel">
								...
								<form id="searchForm" method="post" action="list.ht">
										<ul class="row">
											...
											<select name="Q_allowDel_S" class="select"  value="${param['Q_allowDel_S']}">
												<option value="" <c:if test="${param['Q_allowDel_S']=='' }">selected="selected"</c:if> >--全部--</option>
												<option value="1" <c:if test="${param['Q_allowDel_S'] =='1'}">selected="selected"</c:if> >允许</option>
												<option value="0" <c:if test="${param['Q_allowDel_S'] =='0'}">selected="selected"</c:if> >不允许</option>
											</select></li>
											<li><span class="label">允许编辑:</span>
											<select name="Q_allowEdit_S" class="select"  value="${param['Q_allowEdit_S']}">
												<option value="" <c:if test="${param['Q_allowEdit_S']=='' }">selected="selected"</c:if> >--全部--</option>
												<option value="1" <c:if test="${param['Q_allowEdit_S']=='1' }">selected="selected"</c:if> >允许</option>
												<option value="0" <c:if test="${param['Q_allowEdit_S']=='0' }">selected="selected"</c:if> >不允许</option>
											</select></li>
											<li><span class="label">是否启用:</span>
											<select name="Q_enabled_S" class="select" value="${param['Q_enabled_S']}">
												<option value="" <c:if test="${param['Q_enabled_S']=='' }">selected="selected"</c:if> >--全部--</option>
												<option value="1" <c:if test="${param['Q_enabled_S']=='1' }">selected="selected"</c:if> >是</option>
												<option value="0" <c:if test="${param['Q_enabled_S']=='0' }">selected="selected"</c:if> >否</option>
											</select></li>
				...
				<div class="panel-body">
					    ...
					    <display:table name="sysRoleList" id="sysRoleItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
							...
							<display:column property="category" title="角色类别" style="text-align:left"></display:column>

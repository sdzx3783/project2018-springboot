...
<script type="text/javascript">
	...
	var scriptTree, scriptTypes = [], varTree, isSingleRecord = false, curElement;

	$(function() {
		curElement = dialog.targetEl;
		if (curElement) {
			var funcexp = $(curElement).attr(funcexpAttrStr);
			if (funcexp)
	...
	dialog.onok = function() {
		...
		if (funcexp)
			$(curElement).attr(funcexpAttrStr, funcexp);
		else
			$(curElement).removeAttr(funcexpAttrStr);
	};
	...
	function zTreeOnDblClick(event, treeId, treeNode) {
		...
		var str = isMobileForm ? [ '(data.main.' ] : [ '(m:' ], isMain = treeNode.pId == '0', desc;

		if (!isMain) {
			var angularSubSrc = isSingleRecord ? '(item.' : '(data.sub.' + treeNode.tablename + '.';
			str = isMobileForm ? [ angularSubSrc ] : [ '(s:' ];
		}
			...
			<div class="l-tab-links title-div">
				<div style="width: 100%; height: 100%; font-weight: normal; padding-left: 8px;">
					...
					<label style="margin-left: 8px;">
						<input type="checkbox" onclick="singleSubRecord(this)" />
						子表中单条记录运算
					</label>
				</div>
			</div>

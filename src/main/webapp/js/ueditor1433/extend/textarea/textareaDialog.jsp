...
<script type="text/javascript" src="${ctx}/static/platform/js/ueditor1433/extend/textarea/textareaDialogController.js"></script>
...
<script type="text/javascript">
	...
	$(function() {
		...
		dialog.onok = function() {
			...
			var textarea;
			if (!targetEl) {//新增模式
				var span = $("<span class='textarea' parser='textareaedit'></span>");
				...
				textarea = $(targetEl).find("textarea");
				$(targetEl).attr("external", externalEncode(JSON.stringify(scope.external)));
			}
			textarea.attr("rows",scope.external.rows);
			textarea.attr("cols",scope.external.cols);
			return true;
		};
	});
</script>
		...
		<table class="edit_table">
			<tbody>
				...
				<tr class="style_tr">
					<th>列数:</th>
					<td>
						<input type="text" ng-model="external.cols" style="width: 120px;">
					</td>
					<th>行数:</th>
					<td>
						<input type="text" ng-model="external.rows" style="width: 120px;">
					</td>
				</tr>
			</tbody>
		</table>

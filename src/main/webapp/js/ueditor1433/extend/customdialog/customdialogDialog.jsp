...
<body>
	...
	<div id="inputPanel">
		<div tabid="dialog" title="自定义对话框">
			<fieldset class="base">
				<legend>
					<var id="lang_dialog_setting"></var>
				</legend>
	...
	<script type="text/javascript">
		...
		function bindData(dialog, tabId) {
			...
			for(var i=0;i<dialogQueryArr.length;i++){
					...
					$(":text#"+dialogQueryObj.condition,currentDiv).each(function(){
		...
		function changeConditionParam(jsonObj,currentDiv,type){
			if (jsonObj) {
				...
				for(var i=0,obj;obj=jsonObj[i++];){
					if(obj.defaultType=="1" || obj.defaultType=="4"|| obj.defaultType=="5"){
						obj.defaultValue="";
		...
		dialog.onok = function() {
			...
			if(${not empty param.type}){
				editor.curInput.setAttribute("ht-customerdialog",json);
				var permissionExtend = permissionExtend.split("-");
				//设置权限
				if(isMainTable)	permissionExtend ="{{mian."+permissionExtend[1]+"}}";
				
				else permissionExtend = "{{fields."+permissionExtend[1]+"."+permissionExtend[2]+"}}";
				
				editor.curInput.setAttribute("permission",permissionExtend);
				return ;
			}
			...
			editor.curInput.setAttribute("parser","custombtntable");
		...
		function initFieldsDiv(data){
			...
			for ( var i = 0, c; c = mainTable.fieldList[i++];) {
				...
				// 如果是手机表单
				if(${not empty param.type}){
					var fieldName ="data-main-"+c.fieldName;
				}else{ 
					var fieldName =mainTable.tableName+"-"+c.fieldName;
				}
				
				var itemObj = {name : c.fieldDesc,id : fieldName};
			...
			if(items && selectOptCache.length<=0){
				...
				selectOpt.push(opt);
				for(var i=0, c;c=mainTable.fieldList[i++];){
					 opt='<option value="'+c.fieldName+'"  controlType="'+c.controlType+'" ismain=true >' + c.fieldDesc+ '</option>';
					 selectOpt.push(opt);
				}
			}
			...
			for ( var i = 0, c; c = mainTable.subTableList[i++];) {
				...
				for ( var y = 0, t; t = c.fieldList[y++];) {
					...
					//手机端和电脑端不同
					if(${not empty param.type}){
						var fieldName ="data-"+c.tableName +"-"+t.fieldName
					}else {
						var fieldName =c.tableName+"-"+t.fieldName
					}
					var itemObj = {name : t.fieldDesc,id : fieldName,tableName: c.tableName};
			...
			if(permissionExtend && permissionExtend.length>0){
			...
			if(eventBtnCache.length<1){// 编辑时第一次循环
				if(${not empty param.type}) 
					var dialogStr = $(editor.curInput).get(0).getAttribute('ht-customerdialog');
				else var dialogStr = $(editor.curInput).get(0).getAttribute('eventBtn');
				if (dialogStr) {
		...
		function getFileds(tableId) {
			...
			if(tableId){
				...
				$.ajax({
					url: __ctx + '/platform/form/bpmFormDef/validDesign.ht',
					type: 'POST',
					data:params,
					dataType: 'json',
					async: false,
		...
		function getCurrentTableType(){
			...
			var input = $('input',tbody)[1];
			
			if(!input || input.name.split(":").length < 2){
				var subTable =obj.parents("div[type='subtable']")
				if(subTable.length >0){
					isMainTable = false;
					currentTableName = subTable.attr("tablename").toLowerCase();
				}
				else {
					isMainTable = true;
				}

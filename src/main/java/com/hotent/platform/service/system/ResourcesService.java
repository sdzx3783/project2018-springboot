...
public class ResourcesService extends BaseService<Resources>
{
	...
	public List<Resources> getSysMenu(SubSystem sys,SysUser user){
		...
		if(ContextUtil.isSuperAdmin()){
			...
			resourcesList=resourcesDao.getSuperMenuByAllRoleAndOrg(systemId);
		}else{
			if(StringUtil.isNotEmpty(rolealias)){
//				resourcesList=resourcesDao.getNormMenuByAllRole(systemId,rolealias);
				resourcesList=resourcesDao.getNormMenuByAllRoleAndOrg(systemId,rolealias);
			}else{
				//resourcesList=resourcesDao.getNormMenuByRole(systemId,user.getUserId());
				resourcesList=resourcesDao.getNormMenuByRoleAndOrg(systemId,user.getUserId());
			}
		}
	 ...
	 * @return
	 */
	public List<Resources> getOrgResByParentId(Long id){
		return resourcesDao.getOrgResByParentId(id);
	}

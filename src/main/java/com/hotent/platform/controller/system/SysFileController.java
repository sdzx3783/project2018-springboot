...
import com.hotent.platform.controller.picture.PictureHandlerController;
 ...
 */
@Controller
@RequestMapping(value={"/platform/system/sysFile/","/weixin/system/sysFile/"})
...
public class SysFileController extends BaseController {
	...
	public void fileUpload(MultipartHttpServletRequest request, HttpServletResponse response) throws Exception {
		String preUrl = RequestUtil.getString(request, "preUrl");
		PrintWriter writer = response.getWriter();
		//app调用标识
		boolean isapp = request.getRequestURI().indexOf("/weixin/system/sysFile/")>-1;
		try {
			...
			logger.info("文件上传："+preUrl+";typeId="+typeId);
			boolean mark = true;
			...
			while (it.hasNext()) {
				...
				if(mark){
					...
					if("pictureShow".equals(uploadType)){            //pictureShow控件要的文件路径要有一点不同
						...
						if(typeId==3){
							filePath = createFilePath(attachPath + File.separator +"flow"+ File.separator+ appUser.getAccount(), fileName);
						}else{
							filePath = createFilePath(attachPath + File.separator +"entity"+ File.separator + appUser.getAccount(), fileName);
						}
					...
					*/
					...
					if(isapp){						
						writer.println("{\"return_code\":0,\"return_msg\": \"SUCCESS\",\"data\":{\"fileId\":\"" + fileId + "\",\"fileName\":\"" + oriFileName + "\",\"extName\":\"" + extName + "\"}}");
					}else{
						writer.println("{\"success\":\"true\",\"fileId\":\"" + fileId + "\",\"fileName\":\"" + oriFileName + "\",\"extName\":\"" + extName + "\"}");
					}
					try {
					...
					if(isapp){						
						writer.println("{\"return_code\":4100,\"return_msg\": \"文件格式不符合要求！\",\"data\":{}}");
					}else{
						response.setStatus(500);
			...
			e.printStackTrace();
			if(isapp){						
				writer.println("{\"return_code\":4101,\"return_msg\": \"文件上传失败！\",\"data\":{}}");
			}else{
				response.setStatus(500);
				writer.println("{\"success\":\"false\"}");
			}
		...
		try{			
			...
			while (it.hasNext()) {
				...
				if(uploadName.equals(name)){
					...
					//开始写入物理文件
					String filePath = createFilePath(attachPath,appUser.getAccount(), fileName,fileType);
	 ...
	 * @return 文件的完整目录
	 */
	private String createFilePath(String tempPath,String userAccount,String fileName,String fileType) {
		if(StringUtil.isEmpty(fileType) || !WORD_TEMPLATE.equalsIgnoreCase(fileType)){
			return this.createFilePath(tempPath + File.separator + userAccount,fileName);
		}
		File one = new File(tempPath + File.separator + WORD_TEMPLATE );
		if (!one.exists()) {
			one.mkdirs();
		}
		return one.getPath() + File.separator + fileName;
	...
	public void downloadFile(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(saveType.contains("database")){
			...
			if (StringUtil.isEmpty(filePath))
				return;
			if(PictureHandlerController.PICFILE_TYPEKEY.equals(sysFile.getFileType())){
				attachPath=ServiceUtil.getPictureBasePath();
			}
			if (StringUtil.isEmpty(attachPath)) {
	...
	public void getById(@PathVariable("fileId") Long fileId, HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	public void getPictureById(@PathVariable("fileId") Long fileId, HttpServletRequest request, HttpServletResponse response) throws Exception {
		getFile(request, response, fileId);
	}

	public void getFile(HttpServletRequest request, HttpServletResponse response, Long fileId) throws Exception {
		response.reset();
		String vers = request.getHeader("USER-AGENT");
		SysFile sysFile = sysFileService.getById(fileId);
		String attachPath = "";
		if(PictureHandlerController.PICFILE_TYPEKEY.equals(sysFile.getFileType())){
			attachPath=ServiceUtil.getPictureBasePath().replace("/", File.separator);
		}else{
			attachPath=ServiceUtil.getBasePath().replace("/", File.separator);
		}
		...
		String fileName = sysFile.getFileName() + "." + sysFile.getExt();
		logger.info("附件名称："+fileName+";附件地址："+realPath);
		...
		if("txt".equalsIgnoreCase(sysFile.getExt())){
		...
		if(saveType.contains("database")){
			...
			if (file.exists()) {
				...
				logger.error("附件不存在：fileId="+fileId);
				String message = "<font style='font-weight:800;color:#696969;font-size:14px;text-align:center;'>The file is not exists!</font>";

...
public class SysOrgController extends BaseController {
	...
	@ResponseBody
	public SysOrg getSysOrg(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String orgName = "";
		String orgPathname = sysOrg.getOrgPathname();
		if(StringUtils.isNotEmpty(orgPathname)){
			if("/深水咨询".equals(orgPathname)){
				orgName="深水咨询";
			}else{
				orgPathname = orgPathname.replace("/深水咨询/", "");
			}
			int i = orgPathname.indexOf("/");
			if(i>0){
				orgPathname = orgPathname.substring(0,i);
			}
			if(orgPathname.startsWith("/")){
				orgPathname = orgPathname.substring(1);
			}
			orgName=orgPathname;
		}
	...
	public ModelAndView selector(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String allOrgs = RequestUtil.getString(request, "allOrgs");
		if(!StringUtil.isEmpty(allOrgs)){

...
public class UserPositionController extends BaseController
{
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, "在岗位删除用户成功!");
		}catch(Exception ex){
	 ...
	 * @throws Exception
	 */
	@RequestMapping("del_user_pos")
	@Action(description="删除SYS_USER_POS")
	public void delUserPos(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try{
			Long[] lAryId =RequestUtil.getLongAryByStr(request, "userposid");
			userPositionService.delByIds(lAryId);
			writeResultMessage(response.getWriter(),"在岗位删除用户成功!",ResultMessage.Success);
		}catch(Exception ex){

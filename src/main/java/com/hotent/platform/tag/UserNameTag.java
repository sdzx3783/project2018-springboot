...
public class UserNameTag extends BodyTagSupport {
	...
	private String getUserName() {
		...
		if (this.userId != 0) { // 通过用户Id获取用户名称
			...
			String url = ctx + "/platform/system/sysUser/get.ht?userId="
					+ this.userId + "&canReturn=1&hasClose=true";
			...
			//this.getTaskUser(sb, taskId, ctx, imgSrc);
		}
	...
	/*private void getTaskUser(StringBuffer sb, String taskId2, String ctx,
			String imgSrc) {
		TaskUserService taskUserService = (TaskUserService) AppUtil
				.getBean("taskUserService");
		...
		if (candidateUsers.size() > 5) {
			sb.append("<a  href='#' onclick='return false;' taskId='"
					+ taskId
					+ "' >执行人</a><input type='hidden' name='candidateUsersJson' value='"
					+ JSONArray.fromObject(candidateUsers) + "'>");
		} else {
			String orgUrl = ctx
					+ "/platform/system/sysUserOrg/getUserListByOrgId.ht?orgId=";
			String posUrl = ctx
					+ "/platform/system/sysUser/getUserListByPosId.ht?posId=";
			String roleUrl = ctx
					+ "/platform/system/userRole/getUserListByRoleId.ht?roleId=";
			String jobUrl = ctx
					+ "/platform/system/sysUser/getUserListByJobId.ht?jobId=";
			...
			for (TaskExecutor te : candidateUsers) {
				...
				if (TaskExecutor.USER_TYPE_ORG.equals(te.getType())) {
					...
					sb.append("<a href='" + ctx
							+ "/platform/system/sysUser/get.ht?userId="
							+ te.getExecuteId()
							+ "&canReturn=1&hasClose=true' target='_blank'>"
							+ te.getExecutor() + "</a>");
				}
				if (StringUtils.isNotEmpty(targetUrl))
					sb.append("<a href='#' onclick='return false;' taskId='"
							+ taskId + "' targetUrl='" + targetUrl + "'>"
							+ te.getExecutor() + "</a>");
			}
		}
	}*/
